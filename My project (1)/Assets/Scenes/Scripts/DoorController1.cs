...
{
    public GameObject Door;          // Het child-object "Door" dat de deur zelf is
    public GameObject Doorknob;      // Het child-object "Doorknob" voor de deurknop
    public float openPosition = 5f;  // De positie waar de deur open gaat (verplaatsen langs de X-as)
    ...
    void Start()
    {
        ...
        openPositionVector = new Vector3(closedPosition.x + openPosition, closedPosition.y, closedPosition.z); // De open positie langs de X-as
        
        // Zorg ervoor dat de deurknop altijd zichtbaar is en aan de deur hangt
        Doorknob.transform.SetParent(Door.transform);  // Maak de deurknop een child van de deur
    ...
    void Update()
    {
        ...
        if (isOpen)
        {
            // Verschuif de deur naar de open positie
            Door.transform.position = Vector3.MoveTowards(Door.transform.position, openPositionVector, Time.deltaTime * doorSpeed);
        ...
        {
            // Verschuif de deur terug naar de gesloten positie
            Door.transform.position = Vector3.MoveTowards(Door.transform.position, closedPosition, Time.deltaTime * doorSpeed);
        }
    }
